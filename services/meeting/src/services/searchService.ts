 ...
 */
export async function searchMeetings(filters: SearchFilters): Promise<SearchResult> {
  const {
    query,
    status,
    dateFrom,
 ...
 */
export async function getMeetingStats(userId?: string): Promise<any> {
  ...
  const [
    ...
    // Pending meetings
    Meeting.countDocuments({ ...query, status: { $in: ['pending', 'processing', 'transcribed'] } }),
 ...
 * Get recent meetings
 */
export async function getRecentMeetings(userId: string, limit: number = 10): Promise<IMeeting[]> {
