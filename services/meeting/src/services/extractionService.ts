 ...
 */
async function extractActionItems(transcript: string): Promise<{
  actionItems: RawActionItem[];
 ...
 */
async function extractDecisions(transcript: string): Promise<{
  decisions: RawDecision[];
  tokensUsed: { input: number; output: number };
}> {
  try {
 ...
 */
export async function extractFromTranscript(transcript: string): Promise<ExtractionServiceResult> {
